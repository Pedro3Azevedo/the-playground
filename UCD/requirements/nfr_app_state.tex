...
\section{Non-Functional Requirements by ISO-25010 Category}

\subsection{Reliability}

\subsubsection{NFR-23: Session State Persistence}
...
\subsubsection{NFR-24: Data Sync on App Resume}
...
\subsubsection{NFR-26: App Lifecycle Management}
...
\subsubsection{NFR-27: Cache Invalidation Strategy}
...
\subsection{Interaction Capability}

\subsubsection{NFR-25: UI State Preservation}
\label{nfr:ui-state-preservation}

\begin{tabular}{|l|p{10cm}|}
	\hline
	\textbf{ID} & NFR-25 \\
	\hline
	\textbf{Title} & UI State Preservation \\
	\hline
...
\begin{enumerate}
	...
	\item UI state is cleared when user explicitly logs out
	\item If app process is terminated by the system, UI state is not restored (only session is restored)
	\item Previously entered court or player search text in search bars is accessible via search history
\end{enumerate}

\vspace{0.5cm}

\subsubsection{NFR-29: Form State Recovery}
...
\subsubsection{NFR-28: Background Sync (Optional Enhancement)}
